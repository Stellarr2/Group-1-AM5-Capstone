...
def revenue_over_time_chart(df: pd.DataFrame):
    ...
    # Define point color rule
    revenue_color = (
        alt.when("datum.Amount < 30000")
        .then(alt.value("red"))
...
def monthly_revenue_growth_chart(df: pd.DataFrame):
    """Revenue by month with growth %."""
    data = df.copy()
    data["Order Date"] = pd.to_datetime(data["Order Date"])
    data["Year-Month"] = data["Order Date"].dt.to_period("M").astype(str)

    grouped = data.groupby("Year-Month")["Amount"].sum().reset_index()
    grouped["Growth (%)"] = round(grouped["Amount"].pct_change().fillna(0) * 100)
    
    base = alt.Chart(grouped, title="Monthly Revenue Growth (%)").encode(
        x="Year-Month:N",
    ...
    line = base.mark_line(color="orange", point=True).encode(
        y="Growth:Q"
    )

    return (bars + line).properties(width=500, height=300)
