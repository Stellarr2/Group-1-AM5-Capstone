...
def show_overview(df: pd.DataFrame):
    ...
    st.markdown("---")
    st.title("📘 Streamflow: Make-to-Order (MTO) Process — Overview")
    st.markdown(
        """
This page explains the Make-to-Order (MTO) process used in Streamflow.
"""
    )

    st.header("What is Streamflow?")
    st.markdown(
               """
**Streamflow** is the name of this capstone project — a simulation and dashboard system that models a 
**Make-to-Order (MTO) production flow** from start to finish.  

It ties together the entire lifecycle of an order:

- **Sales Order** → where customer demand begins.  
- **Planned Order** → internal scheduling and resource planning.  
- **Production Order** → manufacturing instructions.  
- **Confirmation** → validation that production is ready.  
- **Delivery** → goods shipped to the customer.  
- **Billing** → invoices generated and tracked.  

What makes Streamflow unique is that it doesn’t just simulate this process — it also provides a **visual dashboard** 
with KPIs, charts, tables, flowcharts, and downloadable reports.  

Think of it as a **miniature ERP system**, built to demonstrate how customer-driven (MTO) production orders flow through 
